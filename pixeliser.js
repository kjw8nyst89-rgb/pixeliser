...
async function initializeLZFSE(){
    ...
    try{
        ...
        if(typeof module._encode_lzfse_memfs!=="function"){
            throw new Error(
                "_encode_lzfse_memfs n'est pas disponible."
            );
        }
        ...
        // ----------------------------------------------------
        // IMPORTANT :
        // l'instance est maintenant disponible globalement.
        // ----------------------------------------------------
        lzfseModule=module;
...
function encodeLZFSE(data){
    ...
    // --------------------------------------------------------
    console.log(
        "Appel encode_lzfse_memfs..."
    );

    const encodedSize=
        lzfseModule._encode_lzfse_memfs(
            data.length
        );

    console.log(
        "Taille compressée retournée :",
        encodedSize
    );

    if(encodedSize<=0){
        throw new Error(
            "Échec de l'encodage LZFSE : "+
            encodedSize
        );
    }
...
class KeyedArchiveResolver{
    ...
    resolveObject(object){
        ...
        if(object["NSRangeCount"]!==undefined&&
        ...
        if(Array.isArray(object["NS.keys"])&&
           Array.isArray(object["NS.objects"])){
            ...
            for(let i=0;i<keys.length;i++){
                ...
                const key=
                    this.resolveObject(keys[i]);

                const value=
                    this.resolveObject(values[i]);
...
function decodeGrilleArchive(decoded){
    ...
    const decoder=
        new BinaryPlistDecoder(decoded);
    ...
    const resolver=
        new KeyedArchiveResolver(plist);
...
currentGrilleFileName =
    ...
    if(
        grilleData.GRILLE instanceof Uint8Array
    ){
    ...
    if(
        grilleData.COULEUR instanceof Uint8Array
    ){
    ...
    if(
        grilleData.PALETTE instanceof Uint8Array
    ){
    ...
    if(
        typeof grilleData.TILESIZE==="number"
    ){
...
function decodeNSIndexSet(value){
    ...
    if(
        integers.length!==expectedIntegerCount
    ){
...
function recomputeGrid(){
    ...
    cols=
        Math.ceil(
            sourceImage.width/tileSize
        );

    rows=
        Math.ceil(
            sourceImage.height/tileSize
        );
...
function resetCamera(){
    ...
    panX=
        (viewport.width-
         worldWidth*zoomFactor)/2;

    panY=
        (viewport.height-
         worldHeight*zoomFactor)/2;
...
function handleTouchMove(event){
    ...
    if(event.touches.length===2){
        ...
        // ----------------------------------------------------
        const worldX=
            (lastPinchCenterX-panX)/
            zoomFactor;

        const worldY=
            (lastPinchCenterY-panY)/
            zoomFactor;
